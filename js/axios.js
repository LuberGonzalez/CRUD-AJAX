...
// Funcion que se encarga de obtener todos los santos.
const getAll = async () => {
  try {
    let res = await axios.get("http://localhost:4000/santos");
    ...
    if (!e.target.id.value) {
      try {
        let options = {
          ...
          res = await axios("http://localhost:4000/santos", options),
          json = await res.data;
      ...
      try {
        let options = {
          ...
          res = await axios(
            `http://localhost:4000/santos/${e.target.id.value}`,
            options
          ),
          json = await res.data;
    ...
    if (isDelete) {
      try {
        let options = {
          ...
          res = await axios(
            `http://localhost:4000/santos/${e.target.dataset.id}`,
            options
          ),
          json = await res.data;
